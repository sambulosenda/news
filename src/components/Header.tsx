...
export default function HeaderOptimizedNew({ categories = [], breakingNews }: HeaderProps) {
  ...
  const featuredNav = allFeatured.slice(0, 5); // Show only 5 main items
  const remainingFeatured = allFeatured.slice(5);
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    // Swipe left to close menu (natural gesture)
    if (swipeDistance > minSwipeDistance && mobileMenuOpen) {
      setMobileMenuOpen(false);
    }
  }, [mobileMenuOpen]);
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-200">
        ...
        {/* Main header - Al Jazeera inspired compact design */}
        <div className="container-wide">
          <div className="flex items-center h-14 lg:h-16">
            {/* Mobile menu button */}
            <button
              type="button"
              className="lg:hidden p-3 -ml-3 hover:bg-gray-100 active:animate-bounce-scale rounded-lg transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
            ...
            <nav className="hidden lg:flex items-center" aria-label="Main navigation">
              <ul className="flex items-center" role="menubar">
                ...
                <li role="none">
                  <Link
                    href="/"
                    className="px-3 py-2 text-xs font-semibold text-gray-800 hover:text-red-700 transition-colors uppercase tracking-wider whitespace-nowrap"
                    role="menuitem"
                  >
                    HOME
                  </Link>
                ...
                <li className="text-gray-300 select-none mx-1.5" aria-hidden="true">|</li>

              {/* Main navigation items */}
              {featuredNav.map((section, index) => {
                  ...
                  <Fragment key={section.slug}>
                    <li className="relative group" role="none">
                      <Link
                        ...
                        className="flex items-center gap-1 px-3 py-2 text-xs font-semibold text-gray-700 hover:text-red-700 transition-colors uppercase tracking-wider whitespace-nowrap"
                        role="menuitem"
                ...
                {moreNav.length > 0 && (
                  ...
                  <div className="absolute top-full right-0 invisible group-hover:visible group-focus-within:visible opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-all duration-200 pt-1 z-50">
        ...
        {mobileMenuOpen && (
          <>
            ...
            <div 
              className="fixed left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-white z-50 lg:hidden overflow-y-auto animate-slide-in-left"
            ...
            >
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <div className="bg-red-700 text-white px-2 py-1 mr-2">
                      <span className="font-display text-lg font-bold">RF</span>
                    </div>
                    <h2 className="text-lg font-bold">Report Focus</h2>
                  </div>
                  <button
                    onClick={() => setMobileMenuOpen(false)}
                    className="p-3 hover:bg-gray-100 active:animate-bounce-scale rounded-lg min-h-[44px] min-w-[44px] flex items-center justify-center"
              ...
              <nav className="p-6" aria-label="Mobile navigation">
                <div className="space-y-2">
                  <Link
                    href="/"
                    className="block px-6 py-5 text-lg font-bold text-gray-900 hover:bg-red-50 hover:text-red-700 active:bg-red-100 rounded-xl transition-all duration-200 min-h-[48px] flex items-center"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Home
                  </Link>
                  ...
                  {navigation.map((section) => {
                    ...
                    return (
                      <div key={section.slug} className="space-y-1">
                        <Link
                          href={`/news/${section.slug}/`}
                          className={`block px-6 py-5 text-lg font-semibold hover:bg-red-50 hover:text-red-700 active:bg-red-100 rounded-xl transition-all duration-200 min-h-[48px] flex items-center ${section.featured && section.slug !== 'breaking-news' ? 'text-gray-900' : 'text-gray-600'}`}
                          onClick={() => setMobileMenuOpen(false)}
                        >
                          {section.name}
                        </Link>
                          ...
                          <div className="ml-4 space-y-1 border-l-2 border-gray-100 pl-4">
                            {section.children?.map((child) => (
                              <Link
                                key={child.slug}
                                href={`/news/${child.slug}/`}
                                className="block px-5 py-4 text-base font-medium text-gray-600 hover:bg-red-50 hover:text-red-700 active:bg-red-100 rounded-lg transition-all duration-200 min-h-[44px] flex items-center"
                                onClick={() => setMobileMenuOpen(false)}
                              >
                                {child.name}
                              </Link>
                ...
                <div className="mt-8 pt-6 border-t border-gray-200 space-y-2">
                  <Link
                    href="/search"
                    className="block px-6 py-5 text-lg font-medium text-gray-700 hover:bg-red-50 hover:text-red-700 active:bg-red-100 rounded-xl transition-all duration-200 min-h-[48px] flex items-center"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Search
                  </Link>
                  ...
                  <Link
                    href="/subscribe"
                    className="block px-6 py-5 text-lg text-white bg-red-700 font-bold hover:bg-red-800 active:bg-red-900 rounded-xl text-center shadow-md transition-all duration-200 mt-4 min-h-[48px] flex items-center justify-center"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Subscribe
                  </Link>
