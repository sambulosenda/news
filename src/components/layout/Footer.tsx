import Link from 'next/link';

const footerLinks = {
  news: [
    { name: 'All Categories', href: '/news/' },
    { name: 'Breaking News', href: '/news/breaking-news/' },
    { name: 'Politics', href: '/news/politics/' },
    { name: 'Business', href: '/news/business/' },
    { name: 'Technology', href: '/news/technology/' },
    { name: 'World', href: '/news/world/' },
    { name: 'Sports', href: '/news/sports/' },
    { name: 'Health', href: '/news/health/' },
    { name: 'Science', href: '/news/science/' },
  ],
  services: [
    { name: 'Load Shedding', href: '/faq/load-shedding' },
    { name: 'Elections Guide', href: '/faq/elections' },
    { name: 'Economy & Finance', href: '/faq/economy' },
    { name: 'Immigration', href: '/faq/immigration' },
    { name: 'Government', href: '/faq/government' },
    { name: 'Crime & Safety', href: '/faq/safety' },
    { name: 'Opinion', href: '/news/opinion/' },
  ],
  company: [
    { name: 'About Us', href: '/about' },
    { name: 'Our Team', href: '/about/team' },
    { name: 'Editorial Standards', href: '/about/standards' },
    { name: 'Ownership', href: '/about/ownership' },
    { name: 'Contact', href: '/contact' },
    { name: 'Ethics Policy', href: '/ethics' },
    { name: 'Corrections', href: '/corrections' },
    { name: 'Accessibility', href: '/accessibility' },
  ],
  connect: [
    { name: 'RSS Feed', href: '/rss.xml' },
    { name: 'Mobile Apps', href: '/mobile-apps' },
  ]
};

const legalLinks = [
  { name: 'Privacy Policy', href: '/privacy' },
  { name: 'Terms', href: '/terms' },
  { name: 'Cookies', href: '/cookies' },
  { name: 'Diversity Policy', href: '/diversity-policy' },
  { name: 'Publishing Principles', href: '/publishing-principles' },
  { name: 'Sitemap', href: '/sitemap.xml' },
];

const socialLinks = [
  { 
    name: 'Facebook', 
    href: 'https://facebook.com/reportfocus',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
      </svg>
    )
  },
  { 
    name: 'Twitter', 
    href: 'https://twitter.com/reportfocus',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
      </svg>
    )
  },
  { 
    name: 'Instagram', 
    href: 'https://instagram.com/reportfocus',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zM5.838 12a6.162 6.162 0 1112.324 0 6.162 6.162 0 01-12.324 0zM12 16a4 4 0 110-8 4 4 0 010 8zm4.965-10.405a1.44 1.44 0 102.881 0 1.44 1.44 0 00-2.881 0z"/>
      </svg>
    )
  },
  { 
    name: 'LinkedIn', 
    href: 'https://linkedin.com/company/reportfocus',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
      </svg>
    )
  },
  { 
    name: 'YouTube', 
    href: 'https://youtube.com/reportfocus',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
      </svg>
    )
  },
  { 
    name: 'WhatsApp', 
    href: 'https://wa.me/reportfocus',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.149-.67.149-.197.297-.767.966-.94 1.164-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>
      </svg>
    )
  },
];

export default function Footer() {
  return (
    <footer className="bg-gray-50 border-t border-gray-200 mt-20" role="contentinfo">
      {/* Compact Al Jazeera style footer with light theme */}
      <div className="container-wide py-8">
        {/* Top Section - Logo and Social */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 pb-6 border-b border-gray-200">
          {/* Logo - Full text like header */}
          <Link href="/" className="flex items-center">
            <h2 className="font-serif text-2xl font-bold text-gray-900">
              Report Focus News
            </h2>
          </Link>

          {/* Social Icons */}
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-500 mr-2 hidden sm:inline">Follow:</span>
            {socialLinks.map((social) => (
              <Link
                key={social.name}
                href={social.href}
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-500 hover:text-red-700 transition-colors"
                aria-label={`Follow us on ${social.name}`}
              >
                {social.icon}
              </Link>
            ))}
          </div>
        </div>

        {/* Middle Section - 4 Column Layout */}
        <div className="py-6 border-b border-gray-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {/* News Column */}
            <div>
              <h3 className="text-xs font-semibold text-gray-900 uppercase tracking-wider mb-3">News</h3>
              <ul className="space-y-2">
                {footerLinks.news.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-sm text-gray-600 hover:text-red-700 transition-colors block"
                    >
                      {link.name}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Services & Opinion Column */}
            <div>
              <h3 className="text-xs font-semibold text-gray-900 uppercase tracking-wider mb-3">Resources</h3>
              <ul className="space-y-2">
                {footerLinks.services.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-sm text-gray-600 hover:text-red-700 transition-colors block"
                    >
                      {link.name}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Company Column */}
            <div>
              <h3 className="text-xs font-semibold text-gray-900 uppercase tracking-wider mb-3">About</h3>
              <ul className="space-y-2">
                {footerLinks.company.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-sm text-gray-600 hover:text-red-700 transition-colors block"
                    >
                      {link.name}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Connect Column */}
            <div>
              <h3 className="text-xs font-semibold text-gray-900 uppercase tracking-wider mb-3">Connect</h3>
              <ul className="space-y-2">
                {footerLinks.connect.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-sm text-gray-600 hover:text-red-700 transition-colors block"
                    >
                      {link.name}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>

        {/* Bottom Section - Legal and Copyright */}
        <div className="pt-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            {/* Legal Links */}
            <nav className="flex flex-wrap gap-x-3 gap-y-1 text-xs">
              {legalLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="text-gray-500 hover:text-red-700 transition-colors"
                >
                  {link.name}
                </Link>
              ))}
            </nav>

            {/* Copyright */}
            <div className="text-xs text-gray-500">
              <p>© 2025 Report Focus News. All rights reserved.</p>
              <p className="mt-1">Independent journalism serving South Africa and Zimbabwe</p>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}